...
run_test "crm-summarizer" python summarize.py --source csv --csv sample/opportunities.csv

cd "$ROOT/customer-interview-analyzer"
run_test "interview-analyzer" python analyze.py --dir sample_transcripts --question "Why is onboarding hard?"
...
run_test "data-contract" python enforce.py --contract contracts/fct_orders.yml --seed

cd "$ROOT/saas-spend-optimizer"
run_test "saas-spend" python optimize.py --subs sample/subscriptions.csv --usage sample/usage.csv

cd "$ROOT/regulatory-guardrail-checker"
run_test "guardrail" python check.py --spec sample_spec.md --regimes gdpr,ccpa,soc2,wcag
